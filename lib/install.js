...
function download() {
  ...
  bin.download(function(err) {
    if (!err) {
      fs.writeFile(path.join(__dirname, "location.js"), "module.exports = '" + bin.localPath().replace(/\\/g, "\\\\") + "'");
    }
  });
}
...
which('wkhtmltopdf', function (er, resolvedPath) {
  ...
  fs.writeFile(path.join(__dirname, "location.js"), "module.exports = '" + binexe.replace(/\\/g, "\\\\") + "'");
